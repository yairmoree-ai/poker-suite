  ...
  for(let i=0;i<S.tableSize;i++){
    ...
    el.innerHTML=`<div class="${cls}" style="width:88px;min-height:64px;box-sizing:border-box" onclick="clickSeat(${i})" oncontextmenu="event.preventDefault();showPlayerHUD(${i})">
      ${seat?.playerId?`
        ...
        ${seat.playerId&&S.btnLocked&&!seat.folded?`<div class="seat-cards-mini" style="display:flex;gap:3px;margin-top:3px;justify-content:center">${[0,1].map(i=>{const c=(seat.cards||[])[i];const openCard=`event.stopPropagation();event.stopImmediatePropagation();openCP('seat${seat.seatIdx}_c${i}')`;return c?`<div onclick="${openCard}" ontouchstart="${openCard}" style="width:20px;height:28px;border-radius:3px;background:#fff;border:1px solid #ddd;display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer"><span style="font-size:8px;font-weight:900;color:${SC[c.suit]};line-height:1">${c.rank}</span><span style="font-size:7px;color:${SC[c.suit]};line-height:1">${c.suit}</span></div>`:`<div onclick="${openCard}" ontouchstart="${openCard}" style="width:20px;height:28px;border-radius:3px;background:rgba(255,255,255,0.08);border:1px dashed rgba(255,255,255,0.2);display:flex;align-items:center;justify-content:center;cursor:pointer"><span style="font-size:10px;color:rgba(255,255,255,0.25)">+</span></div>`;}).join('')}</div>`:''}
