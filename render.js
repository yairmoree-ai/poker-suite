...
if(i===3&&!S.board[2]){notify('צריך פלופ קודם');return;}
    ...
    btn.innerHTML=card
      ?`<span style="font-size:${Math.round(sw*.5)}px;font-weight:900;color:${SC[card.suit]};line-height:1.1">${card.rank}</span><span style="font-size:${Math.round(sw*.28)}px;color:${SC[card.suit]};line-height:1">${card.suit}</span>`
